...
class ExportController extends Controller
{
    public function laporanStatistik(Request $request)
    {
        ...
        $trendPendaftaran = DB::table('pendaftar')
            ...
            ->whereBetween('tanggal_daftar', [$tanggalMulai, $tanggalAkhir])
            ->groupBy('tanggal')
            ->orderBy('tanggal')
            ->get();
        
        $perbandinganJurusan = DB::table('pendaftar')
        ...
        $perbandinganGelombang = DB::table('pendaftar')
        ...
        $totalPendaftar = DB::table('pendaftar')->whereBetween('tanggal_daftar', [$tanggalMulai, $tanggalAkhir])->count();
        $lulusAdm = DB::table('pendaftar')->whereBetween('tanggal_daftar', [$tanggalMulai, $tanggalAkhir])->where('status', 'ADM_PASS')->count();
        $terbayar = DB::table('pendaftar')->whereBetween('tanggal_daftar', [$tanggalMulai, $tanggalAkhir])->where('status', 'PAID')->count();
        
        $rasioLulus = $totalPendaftar > 0 ? ($lulusAdm / $totalPendaftar * 100) : 0;
        $konversiPembayaran = $totalPendaftar > 0 ? ($terbayar / $totalPendaftar * 100) : 0;
        
        $asalSekolah = DB::table('pendaftar_asal_sekolah')
            ->join('pendaftar', 'pendaftar_asal_sekolah.pendaftar_id', '=', 'pendaftar.id')
            ->select('pendaftar_asal_sekolah.nama_sekolah', 'pendaftar_asal_sekolah.kabupaten', DB::raw('COUNT(*) as jumlah'), DB::raw('AVG(pendaftar_asal_sekolah.nilai_rata) as rata_nilai'))
            ->whereBetween('pendaftar.tanggal_daftar', [$tanggalMulai, $tanggalAkhir])
        ...
        return view('admin.laporan.statistik', compact('trendPendaftaran', 'perbandinganJurusan', 'perbandinganGelombang', 'totalPendaftar', 'lulusAdm', 'terbayar', 'rasioLulus', 'konversiPembayaran', 'asalSekolah', 'tanggalMulai', 'tanggalAkhir'));
    }
